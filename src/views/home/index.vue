<template>
  <div class="h-full flex flex-col lg:flex-row">
    ...
    >
      ...
      <el-text
        truncated
        class="flex-1 text-sm font-semibold leading-6 text-gray-900"
      >
        {{
...
import { useChatListApi, useChatDeleteAllApi } from "@/api/chat";
...
const handleOpenChat = (chat) => {
  ...
  key.value = nanoid();
  router.push("/c/" + currentChatId.value)
...
const handleDeleteAll = () => {
  ElMessageBox.confirm("确定全部清除吗?", "提示", {
    ...
    .then(async () => {
      try {
        const response = await useChatDeleteAllApi("/chat/all", "DELETE");
